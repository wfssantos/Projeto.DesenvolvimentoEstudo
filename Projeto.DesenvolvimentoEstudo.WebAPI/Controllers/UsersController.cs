...
public class UsersController : BaseController
{
    ...
    public async Task<IActionResult> GetAll([FromQuery] GetAllUserRequest filter)
    {
        var command = _mapper.Map<GetAllRequest>(filter);
        var result = await _mediator.Send(new ListUserQuery(command));
        return Ok(result);
    }
